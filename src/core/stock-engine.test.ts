...
vi.mock("@/lib/recipe-parser", () => ({
  resolveRecipeFootprint: vi.fn().mockResolvedValue([{ sku: "MOCK_INGR_1", quantity: 1 }]),
}));

vi.mock("drizzle-orm", () => ({
  eq: (...args: any[]) => args,
  and: (...args: any[]) => args, // Added missing and
...
import { getAllCurrentStock, getCurrentStock, recordTransaction } from "./stock-engine";
...
describe("Stock Engine — Ledger Pattern", () => {
  ...
  it("devuelve un Map con stock de múltiples SKUs", async () => {
    mockRows.push({ sku: "PAN_PAPA", total: "100" }, { sku: "QUESO_CHEDDAR", total: "25.5" });
