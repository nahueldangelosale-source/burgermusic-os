...
import {
  ...
  QuantityStepper,
  StackCard,
} from "@/components/ui/AntigravityAtoms";
import { ArrowRight, ChefHat, Package, Save, Search } from "lucide-react";
...
export default function RecipeBuilder({
  ...
  React.useEffect(() => {
    if (selectedProduct && existingRecipes[selectedProduct.id]) {
      ...
      const dedupedListeners = Array.from(uniqueMap.entries()).map(([id, quantity]) => ({
        id,
        quantity,
      }));
  ...
  const handleSave = async () => {
    ...
    await saveRecipe(
      selectedProduct.id,
      currentRecipe.map((i) => ({ ingredientSku: i.id, quantity: i.quantity })),
    );
    setIsSaving(false);
      ...
      <div className="lg:col-span-8 flex flex-col h-full">
        {selectedProduct ? (
          <>
            ...
            <div className="flex justify-between items-end mb-6">
              ...
              <div className="text-right">
                ...
                <div className="text-3xl font-black text-ink-900 font-mono">
                  {formattedCurrency.format(totalCost)}
                </div>
              </div>
            </div>
                  ...
                  {currentRecipe.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 text-center p-8 border-2 border-dashed border-slate-200 rounded-xl">
                      ...
                      <p className="font-medium">
                        Arrastra o selecciona ingredientes del panel derecho
                      </p>
                    </div>
                  ) : (
                    currentRecipe.map((item) => {
                      ...
                      return (
                        <StackCard
                          key={item.id}
                          className="bg-white border-brand-200 shadow-sm"
                          active
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              ...
                              <div className="text-xs text-slate-400 font-mono">
                                {formattedCurrency.format((ing.costCents || 0) / 100)} / {ing.unit}
                              </div>
                            </div>
                            <QuantityStepper
                              value={item.quantity}
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-8">
            ...
            <h2 className="text-2xl font-black text-slate-300 uppercase tracking-tight">
              El Laboratorio
            </h2>
            <p className="text-slate-400 max-w-md mt-2">
              Selecciona un producto del menú lateral para comenzar a diseñar su composición y
              costos.
            </p>
          </div>
        )}
