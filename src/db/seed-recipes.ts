...
async function main() {
    ...
    const insumos = [
        { id: "INS_CARNE_110", name: "Medallón Carne 110g", unit: "unidad", costCents: 80000 }, // Ej: $800
        { id: "INS_PAN_PAPA", name: "Pan de Papa", unit: "unidad", costCents: 30000 },
        { id: "INS_CHEDDAR", name: "Feta Cheddar", unit: "unidad", costCents: 15000 },
        { id: "INS_BACON", name: "Feta Bacon", unit: "unidad", costCents: 20000 },
        { id: "INS_PAPA_KG", name: "Papas Bastón (Bolsa)", unit: "kg", costCents: 120000 },
        { id: "INS_ACEITE", name: "Aceite Freidora", unit: "litros", costCents: 500000 },
    ];
    ...
    for (const receta of recetas) {
        ...
        const productExists = await db.select().from(products).where(eq(products.id, receta.productSku)).limit(1);

        if (productExists.length === 0) {
            console.warn(`⚠️ Saltando receta para '${receta.productSku}': No existe en productos (¿Cargaste el CSV primero?).`);
