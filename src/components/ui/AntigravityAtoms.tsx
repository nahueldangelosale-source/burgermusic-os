...
export function StackCard({ children, className, onClick, active }: { children: React.ReactNode; className?: string, onClick?: () => void, active?: boolean }) {
    return (
        <div
            onClick={onClick}
            className={cn(
        ...
        >
            {active && (
                <div className="absolute -top-3 -right-3 bg-brand-500 text-white rounded-full p-1 shadow-sm animate-in zoom-in">
                    <svg /* Check Icon */ width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
...
export function QuantityStepper({ value, onChange, unit }: { value: number, onChange: (val: number) => void, unit: string }) {
    return (
        <div className="flex items-center gap-2 bg-slate-50 rounded-lg p-1 border border-slate-200">
            <button
                onClick={(e) => { e.stopPropagation(); onChange(Math.max(0, value - 0.1)); }}
            ...
            <div className="min-w-[4rem] text-center">
                <div className="text-lg font-black text-ink-900 leading-none">{value.toFixed(1).replace('.0', '')}</div>
                <div className="text-[10px] uppercase font-bold text-slate-400 leading-none mt-0.5">{unit}</div>
            </div>
            <button
