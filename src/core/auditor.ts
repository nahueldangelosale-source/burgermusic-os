// src/core/auditor.ts
import { db } from "@/db";
import { products, recipes, inventorySnapshots, transactions } from "@/db/schema";
...
export async function runDailyAudit(date: string): Promise<AuditItem[]> {
    ...
    const dailySales = await db.select().from(transactions).where(and(
        eq(transactions.date, date),
        eq(transactions.type, "SALE")
    ));

    // 2. Obtener Stock Final del día (Reportado por WhatsApp)
    const endStock = await db.select().from(inventorySnapshots).where(
    ...
    for (const sale of dailySales) {
        // Buscar receta
        const productRecipes = await db.select().from(recipes).where(
            eq(recipes.productSku, sale.productSku)
        );
    ...
    const allSkus = new Set([...theoreticalUsage.keys(), ...endStock.map(i => i.productSku)]);
