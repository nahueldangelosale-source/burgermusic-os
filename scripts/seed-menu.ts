...
import { products, recipes } from "../src/db/schema";
...
async function main() {
    ...
    for (const category of menuData) {
        ...
        for (const item of category.products) {
            ...
            if (category.category.includes("Hamburguesas") && !desc.includes("PAN DE QUESO") && !desc.includes("CROISSANT")) {
            ...
            await db.delete(recipes).where(eq(recipes.productSku, sku));
